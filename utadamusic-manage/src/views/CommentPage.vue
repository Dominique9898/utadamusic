    ...
    methods: {
        getData() {
            ...
            if (_this.$route.query.type === 0) {
                url = this.$store.state.HOST+'/songComments?songId='
            } else if (_this.$route.query.type === 1){
                url = this.$store.state.HOST+'/songListComments?songListId='
            }
        ...
        getUsers (id, item) {
            let _this = this
            _this.$axios.get(this.$store.state.HOST+'/commentOfConsumer?id=' + id).then(function (res) {
                    let o = item
        ...
        deleteRow(){
            var _this = this
            _this.$axios.get(this.$store.state.HOST+'/api/deleteComments?id=' + _this.tableData[_this.idx].id)
                .then(response => {
                    if (response.data) {
                        _this.getData()
                        _this.$notify({
