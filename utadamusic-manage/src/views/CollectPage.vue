...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            _this.$axios.get(this.$store.state.HOST+'/myCollection?userId=' + this.$route.query.id).then((res) => {
                _this.tableData = []
        ...
        deleteRow(){
            var _this = this
            _this.$axios.get(this.$store.state.HOST+'/api/deleteCollects?id=' + _this.tableData[_this.idx].id)
                .then(response => {
                    if (response.data) {
                        _this.getData();
                        _this.$notify({
