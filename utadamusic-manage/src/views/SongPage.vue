...
export default {
    ...
    methods: {
        ...
        getData() {
            var _this = this
            _this.tableData = []
            _this.$axios.get(this.$store.state.HOST+'/AllSongs').then((res) => {
        ...
        getSingerName () {
            ...
            _this.$axios.get(this.$store.state.HOST+'/searachSingers?name=' + value).then(function (res) {
                if (!res.data.length) {
                    _this.$notify({
                        title: '系统暂无该该歌手',
                        type: 'warning'
        ...
        addSong(id){
            ...
            req.open("post", this.$store.state.HOST+'/api/addSong', false)
            req.send(form);
        ...
        deleteRow(){
            var _this = this
            _this.$axios.get(this.$store.state.HOST+'/api/deleteSongs?id=' +  _this.tableData[_this.idx].id)
                .then(response => {
                    if (response.data) {
                        _this.getData()
                        _this.$notify({
