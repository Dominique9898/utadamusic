...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            _this.tableData = []
            _this.tempDate = []
            _this.$axios.get(this.$store.state.HOST+'/listSongLists').then((res) => {
        ...
        saveEdit() {
            ...
            _this.$axios.post(this.$store.state.HOST+'/api/updateSongListMsgs', params)
                .then(res => {
                    if (res.data.code === 1) {
                        _this.$notify({
                            title: '编辑成功',
        ...
        addsongList () {
            ...
            _this.$axios.post(this.$store.state.HOST+'/api/addSongList', params).then(res => {
                    if (res.data.code === 1) {
                        _this.getData()
                        _this.registerForm = []
                        _this.$notify({
        ...
        deleteRow () {
            let _this = this
            _this.$axios.get(this.$store.state.HOST+'/api/deleteSongLists?id=' + _this.tableData[this.idx].id)
                .then(res => {
                    if (res.data) {
                        _this.getData()
                        _this.$notify({
